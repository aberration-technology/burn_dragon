...
fn assert_report_sane(report: &OptimizerDynamicsReport, min_loss_delta: f32) {
    ...
    assert!(
        report.initial_train_loss.is_finite(),
        "initial train loss must be finite: {report:?}"
    );
    assert!(
    ...
    assert!(
        ...
        "final train loss must be finite: {report:?}"
    );
    assert!(
        report.train_loss_delta() > 0.0,
        "optimizer should reduce train loss during dynamics bench: {report:?}"
    );
    assert!(
        report.loss_delta() >= min_loss_delta,
        "optimizer should stay within the configured dynamics bound during bench: {report:?}"
    );
}
...
fn bench_optimizer_dynamics(c: &mut Criterion) {
    ...
    c.bench_function(
        "optimizer_dynamics/eggroll_rank_sgd_pop256_rank4_tiny_next_token",
        |b| {
            b.iter(|| {
                let dir = tempdir().expect("tempdir");
                let report =
                ...
                assert_report_sane(&report, -0.05);
                black_box(report)
            })
        },
    );
