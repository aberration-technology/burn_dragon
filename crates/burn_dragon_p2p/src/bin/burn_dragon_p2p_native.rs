...
const DEFAULT_MAINNET_REVISION_ID: &str = "nca-r4";
...
fn run_trainer_daemon(args: RunTrainerDaemonArgs) -> Result<()> {
    let mut config = resolved_config(
        args.config.as_deref(),
        args.config_format,
        args.edge_url,
...
fn contains_native_trainer_role(roles: &PeerRoleSet) -> bool {
    roles.contains(&PeerRole::TrainerCpu) || roles.contains(&PeerRole::TrainerGpu)
}

fn trainer_daemon_step_eligible(
    roles: &PeerRoleSet,
