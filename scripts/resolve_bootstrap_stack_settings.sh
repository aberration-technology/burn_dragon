...
resolve_deploy() {
  ...
  if [ -z "$bootstrap_version" ]; then
    bootstrap_version="0.21.0-pre.72"
  fi
...
PY
  ...
  auth_principals_json="$(python3 -c '''import json, sys; principals = json.loads(sys.argv[1] or "[]"); principal_ids = set(sys.argv[2:]); principals = [item for item in principals if item.get("principal_id") not in principal_ids]; print(json.dumps(principals))''' "$auth_principals_json" "$native_canary_principal_id" "$native_canary_validator_principal_id")"
