...
fn runtime_sync_contract() -> Result<()> {
    let commands = render_bootstrap_runtime_sync_commands(&RuntimeCommandEnv {
        ...
        bootstrap_crate_version: "0.21.0-pre.79".to_owned(),
    ...
    require_contains(
        &joined,
        "cargo install --locked burn_p2p_bootstrap --version '0.21.0-pre.79'",
...
fn native_canary_contracts() -> Result<()> {
    ...
    for snippet in [
        ...
        "cargo run -p xtask -- run-live-native-training-canary",
        "cargo run -p xtask -- summarize-live-native-training-canary",
        ...
        "BURN_DRAGON_NATIVE_CANARY_CANONICAL_TIMEOUT_SECS: \"480\"",
        "BURN_DRAGON_NATIVE_CANARY_P2P_TIMEOUT_SECS: \"300\"",
    ] {
