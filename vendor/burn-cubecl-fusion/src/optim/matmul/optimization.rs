...
impl<'a, R: Runtime> Vectorization<R> for FusedMatmulLaunch<'a> {
    fn minimum_vectorization_element_size(&self) -> usize {
        // A byte mask in the epilogue must not over-vectorize floating-point tiles.
        // Packed quantized inputs retain the quantization planner's separate rules.
        self.matmul
            .lhs
            .precision()
            .into_storage_type()
            .size()
            .max(self.matmul.rhs.precision().into_storage_type().size())
            .max(self.matmul.out.precision().into_storage_type().size())
    }
